...
if (!empty($_POST["data"])) {
    ...
    if(!empty($file["movImage"]["tmp_name"])) {
        move_uploaded_file($file["movImage"]["tmp_name"], "uploads/" .basename($file["movImage"]["name"]));
    }

    $sql = "INSERT INTO movie (movName, movDescription, movRating, movDate, movGenre, movActors, movAge, movTags, movImage) VALUES(:movName, :movDescription, :movRating, :movDate, :movGenre, :movActors, :movAge, :movTags, :movImage)";
    $bind = [":movName" => $data["movName"],
        ":movDescription" => $data["movDescription"],
        ":movRating" => $data["movRating"],
        ":movDate" => $data["movDate"],
        ":movGenre" => $data["movGenre"],
        ":movActors" => $data["movActors"],
        ":movAge" => $data["movAge"],
        ":movTags" => $data["movTags"],
        ":movImage" => (!empty($file["movImage"]["tmp_name"])) ? $file["movImage"]["name"] : NULL,
    ];
    ...
    <form method="post" action="insert.php" enctype="multipart/form-data">
        <div class="row">
            ...
            <div class="col-12 mb-4">
                <label class="form-label" for="movImage">Image</label>
                <input type="file" class="form-control" id="movImage" name="movImage">
            </div>
